...
enum class TypeUiModel1(
    ...
    companion object {
        fun fromId(id: Int): TypeUiModel1 {
            return entries.find { it.id == id } ?: throw IllegalArgumentException("Unknown type ID: $id")
        }
    }
}

fun Type.toUi(): TypeUiModel1 = TypeUiModel1.valueOf(this.name)

fun TypeUiModel1.toData(): Type = Type.fromId(this.id)

fun List<Type>.toUi(): List<TypeUiModel1> = map(Type::toUi)
